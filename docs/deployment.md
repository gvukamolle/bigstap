...
После того как приложение поднялось и схема создана, **выключаем push** (чтобы прод не «дрейфил» схему):
...
(на них завязаны редирект bootstrap и rate-limit). DNS A-запись на IP сервера должна уже существовать
(шаг 9), иначе Let's Encrypt не выпустит сертификат — при необходимости сделайте шаг 9 заранее.
...
Схема БД при обновлении не трогается (push выключен). При изменениях схемы Payload — временно
включить `PAYLOAD_DB_PUSH=true` (как на шаге 6) или применить миграции, когда они появятся.
...
```bash
# Дамп Postgres
docker compose exec -T postgres pg_dump -U bigstep bigstep > backup-$(date +%F).sql

# Восстановление
cat backup-YYYY-MM-DD.sql | docker compose exec -T postgres psql -U bigstep -d bigstep

# Загрузки Payload (том bigstep-media)
docker run --rm -v bigstep_bigstep-media:/data -v "$PWD":/backup alpine \
  tar czf /backup/media-$(date +%F).tar.gz -C /data .
```

> Имя тома — с префиксом проекта (по умолчанию имя каталога), проверьте через `docker volume ls`.
> Настройте регулярный бэкап (cron) и храните копии вне сервера.
