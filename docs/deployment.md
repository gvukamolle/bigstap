...
| `PAYLOAD_DB_PUSH` | оставить **пустым** (в production-сборке push не работает вовсе — схема создаётся на шаге 6 через туннель) |
| `YOOKASSA_*`, `CDEK_*` | можно оставить пустыми: ключи удобнее вставить после запуска в админке — Настройки → «Интеграции: оплата и доставка» (приоритет над env, применяются без перезапуска) |
...
```bash
# Собрать образ и поднять стек
docker compose up -d --build
docker compose ps
curl -I http://127.0.0.1:3000/   # ожидаем HTTP/1.1 200
```

На чистой Postgres таблиц ещё нет, и сами они **не создадутся**: drizzle `push` у Payload работает
**только в dev-режиме** (`next dev`) — production-сборка игнорирует `PAYLOAD_DB_PUSH=true` молча,
без ошибок в логах (проверено на живом деплое 12.06.2026: API отвечает, но любой запрос к коллекции
падает с `relation "products" does not exist`).

Пока в репозитории нет миграций, схему создаёт **локальный dev-инстанс Payload с рабочей машины**
через SSH-туннель к серверной Postgres:

```bash
# 1) НА СЕРВЕРЕ: временно опубликовать порт Postgres на localhost
cd ~/bigstap
printf 'services:\n  postgres:\n    ports:\n      - "127.0.0.1:5432:5432"\n' > docker-compose.override.yml
docker compose up -d postgres

# 2) НА РАБОЧЕЙ МАШИНЕ (в каталоге репозитория, node_modules установлены): туннель
ssh -f -N -L 15432:127.0.0.1:5432 deploy@SERVER_IP

# 3) НА РАБОЧЕЙ МАШИНЕ: одноразовый скрипт пуша (dev-режим => push сработает)
cat > tmp-push-schema.mts <<'EOF'
import { getPayload } from 'payload'
import config from './payload.config'
const payload = await getPayload({ config })
const users = await payload.count({ collection: 'users' })
console.log('OK, схема создана. users count:', users.totalDocs)
process.exit(0)
EOF
DATABASE_URI="postgres://bigstep:ПАРОЛЬ@127.0.0.1:15432/bigstep" \
  PAYLOAD_SECRET="local-push-secret-only-for-schema" npx tsx tmp-push-schema.mts
rm tmp-push-schema.mts && pkill -f "ssh.*15432"

# 4) НА СЕРВЕРЕ: закрыть порт обратно и перезапустить прод
rm docker-compose.override.yml
docker compose up -d
```

Проверка, что таблицы появились и API работает:

```bash
docker compose exec -T postgres psql -U bigstep -d bigstep -c '\dt'   # users, orders, products, ...
curl -s 'http://127.0.0.1:3000/api/products?limit=1'                  # JSON без errors
```

`PAYLOAD_DB_PUSH` в `.env` держите **пустым** — в production он всё равно не работает, а оставленный
`true` только маскирует проблему при будущем переходе на dev-окружение рядом с прод-базой.

> Когда появится полноценный workflow миграций Payload, этот шаг заменяется на `payload migrate`
> внутри контейнера, и туннель больше не нужен.
...
Схема БД при обновлении не трогается (production-сборка не умеет push). **Перед любым изменением
схемы обязательно сделайте бэкап** (`pg_dump`, раздел 11). Безопасный путь — Payload migrations,
когда они появятся. До этого порядок строго: бэкап → dev-push через SSH-туннель (процедура из
шага 6) → проверить таблицы (`\dt`).
